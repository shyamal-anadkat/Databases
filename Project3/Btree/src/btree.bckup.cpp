...
BTreeIndex::BTreeIndex(const std::string&relationName,
    ...
    this->leafOccupancy = 7;
    this->nodeOccupancy = 8;
...
const void BTreeIndex::insertEntry(const void *key, const RecordId rid) {
    ...
    if (splitData) {
        ...
        // std::cout << "(Root) New Node: " << rootPageNum << "\n";
        bufMgr->unPinPage(file, headerPageNum, true);
        bufMgr->unPinPage(file, rootPageNum, true);
    }
}
...
SplitData <int> *BTreeIndex::insertEntry(PageId pageNum,
    ...
    for (index = 0; index <= lastIndex && key >= nonLeafNode->keyArray[index];
         index++)
        ;
    // std::cout << "Current Page : " << pageNum << "\n";
    // std::cout << "key : " << key << "\n";
    // std::cout << "Current node array : \n\t";
    // for (int i = 0; i < nodeOccupancy; i++) {
    ...
    PageId nextPageNum = nonLeafNode->pageNoArray[index];
    // std::cout << "nextIsLeaf : " << isNextLeaf << "\n";
    ...
    SplitData <int> *splitPointer =
        insertEntry(nextPageNum, ridKeyPair, isNextLeaf);
...
SplitData <int> *BTreeIndex::splitNonLeafNode(PageId pageNum,
    ...
    for (int idx = 0; idx <= nodeOccupancy; idx++) {
    ...
    if (pIdx < halfIndex) {
        ...
        // std::cout << "(Non Leaf 1) New Node: " << newNodeData->newPageId << "\n";
        // return the split data
        return newNodeData;
    }
    // If the split happens at the new key
        ...
        // std::cout << "(Non Leaf 2) New Node: " << newNodeData->newPageId << "\n";
        return newNodeData;
    }
...
// std::cout << "(Non Leaf 3) New Node: " << newNodeData->newPageId << "\n";
        return newNodeData;
    }
...
SplitData <int> *BTreeIndex::splitLeafNode(struct LeafNodeInt *leafNode,
    ...
    /*if (found999) {
        ...
        std::cout << "\n\t";
        for(int i= 0; i < leafOccupancy; i++) {
            std::cout << leafNode->ridArray[i].page_number << "/" << leafNode->ridArray[i].slot_number << " ";
        }
        std::cout << "\n";
        std::cout << "pIdx : " << pIdx << "\n";
    }*/
    ...
    if (pIdx < halfIndex) {
        ...
        /*if (found999) {
            ...
            std::cout << "New Leaf: \n\t";
            for(int i= 0; i < leafOccupancy; i++) {
                std::cout << newLeaf->keyArray[i] << " ";
            }
            std::cout << "\n\t";
            for(int i= 0; i < leafOccupancy; i++) {
                std::cout << newLeaf->ridArray[i].page_number << "/" << newLeaf->ridArray[i].slot_number << " ";
            }
            std::cout << "\n";
        }*/
        ...
        // std::cout << "(Leaf) New Node: " << splitData->newPageId << "\n";
        return splitData;
    }
...
const void BTreeIndex::startScan(const void *lowValParm,
    ...
    while (!stop) {
        ...
        if (sIdx == leafOccupancy || !(leaf->ridArray[sIdx].page_number)) {
 ...
 *        // valid page number at i+1 in the pageNoArray
